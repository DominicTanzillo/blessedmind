...
export default function DashboardView({
  ...
  const outletCtx = useOutletContext<{ showOnFocus?: boolean; toggleShowOnFocus?: () => void }>() ?? {}
  ...
  return (
    <div className="space-y-6 py-2">
      ...
      <div className="text-center space-y-3">
        ...
        {/* Sticky notes toggle — desktop only */}
        {outletCtx.toggleShowOnFocus && (
          <button
            onClick={outletCtx.toggleShowOnFocus}
            className="hidden lg:inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs text-stone-400 hover:text-amber-600 hover:bg-amber-50 border border-transparent hover:border-amber-200 transition-all duration-300"
          >
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
            </svg>
            {outletCtx.showOnFocus ? 'Hide sticky notes' : 'Show sticky notes'}
          </button>
        )}
      </div>
    </div>
